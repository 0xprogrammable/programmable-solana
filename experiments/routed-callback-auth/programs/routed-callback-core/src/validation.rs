...
pub fn validate_user_source_before_authorization(
    account: &TokenAccount,
    expected_mint: Pubkey,
    expected_authority: Pubkey,
) -> Result<()> {
    ...
    require!(
        account.delegate == COption::None,
        CoreError::ExistingTokenDelegate
    );
    require!(
        account.delegated_amount == 0,
        CoreError::InvalidDelegatedAmount
...
pub fn validate_delegated_spend_source(
    account: &TokenAccount,
    expected_mint: Pubkey,
    expected_spend_authority: Pubkey,
    exact_total_debit: u64,
) -> Result<()> {
    validate_common_token_account(account, expected_mint)?;
    require!(
        account.delegate == COption::Some(expected_spend_authority),
        CoreError::InvalidSpendAuthority
    );
    require!(
        account.delegated_amount == exact_total_debit,
