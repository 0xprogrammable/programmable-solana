...
readonly build_dir="$(mktemp -d /tmp/programmable-routed-callback-auth-sbf.XXXXXX)"
...
ensure_artifact_dir_is_pure() {
  unexpected_entry="$(
    find "${artifact_dir}" -mindepth 1 -maxdepth 1 \
      ! \( \
        -name 'programmable_routed_callback_core.so' -o \
        -name 'routed_plan_engine.so' -o \
...
build_program programs/routed-callback-core/Cargo.toml
build_program test-programs/routed-plan-engine/Cargo.toml
...
install -m 0644 \
  "${build_dir}/programmable_routed_callback_core.so" \
  "${artifact_dir}/programmable_routed_callback_core.so"
install -m 0644 \
  "${build_dir}/routed_plan_engine.so" \
  "${artifact_dir}/routed_plan_engine.so"
install -m 0644 \
  "${build_dir}/hostile_router_probe.so" \
  "${artifact_dir}/hostile_router_probe.so"
install -m 0644 \
  "${build_dir}/callback_capability_probe.so" \
  "${artifact_dir}/callback_capability_probe.so"
